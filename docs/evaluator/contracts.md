...
| `configurationHash`               | Hash of `EvaluationPlanConfigurationPreimage`; it never hashes the containing plan or its own field                                     |
...
All deterministic JSON preimages use the versioned algorithm
`obby-canonical-json-v1` and SHA-256. This design does not claim RFC 8785 compliance.
...
- `schemaVersion`, `executionId`, `planId`, `manifestHash`, `planHash`, `configurationHash`;
...
- `canonicalizationAlgorithm`, calculation-bundle schema version, `manifestHash`, `planHash`,
...
- `scene`, `plan`, and `versions` including manifest, plan/configuration, evaluator,
