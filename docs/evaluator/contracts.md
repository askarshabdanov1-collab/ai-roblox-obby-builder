...
| `ControllerProfile` | Versioned coarse limits, avatar dimensions, supported surfaces, tolerance policy, constant classifications, and `controllerProfileHash` |
...
E1b defines a closed controller/avatar profile for deterministic coarse analysis. Every limit is
...
| `controllerProfileHash` | `ControllerProfilePreimage`               | Schema/profile/model versions, classified limits, avatar dimensions, landing margin, supported surfaces, tolerance, limitations | Own hash, timestamps, execution/session IDs, host/environment/storage metadata                           | Supported surfaces and limitations sort lexicographically                         | No timestamps                                                                                  | No execution/session/random IDs            | No live host/engine environment                                              | Same semantic controller profile gives the same hash                         |
