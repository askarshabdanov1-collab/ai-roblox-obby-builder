...
`MetricCalculationPreimage` contains `canonicalizationAlgorithm`, schema version, metric ID and
version, `metricDefinitionHash`, calculation-configuration identity, explicit calculation state,
deterministic parameters, `evidenceContentHash` values sorted by evidence
kind, subject stable key, then hash, parent metric `calculationHash` values sorted by parent metric
ID then hash, typed result/status or typed unavailable reason, blocking invariant/evidence when
applicable, thresholds applied, confidence calculation inputs/result, limitations sorted by stable
limitation code then text, and reproduction method/input hashes. It excludes its own `calculationHash`,
...
- `outcome`, sorted `blockingFindingIds`, `scoreProfile`, canonically ordered metric calculations,
  invariant-gate and profile-gate results, explicit completeness, derived metric views, findings,
  `evidenceIndex`, availability-record hashes, `missingEvidence`, `comparability`, and limitations.
...
| `reportPayloadHash`     | `ReportPayloadPreimage`                   | `calculationBundleHash`, scene/plan/versions, outcome, profile, calculations/gates/completeness, metrics/findings/evidence/availability/missing/comparability/limitations | Own hash, execution/export fields, paths, render/UI metadata                                                           | Canonical calculation/gate/metric/finding/evidence orders above                   | No timestamps                                                                                  | No execution/random IDs                    | Only identities already bound by calculation/evidence                        | Equivalent results across executions must match                              |
...
| `availabilityRecordHash`         | `AvailabilityRecordPreimage`            | Subject, state, reason, authority, producer, semantic effective point, supersession parents, policy, successor, impact scope | Own hash, storage/cache/UI metadata, mutable notes, processing/receipt time          | Parent/affected hashes and reason details sorted by their stable keys | Normalized semantic effective time included; storage/processing times excluded    | No execution/session/random IDs                           | Semantic authority and producer included; host/storage metadata excluded | Same immutable availability assertion matches  |
...
- `producer`, a `VersionRef` containing the record-producing component, semantic version, and
  optional immutable build/configuration hash;
...
Producer identity is distinct from authority and policy identity. A producer change changes the
record hash. E1c rejects incomparable producer/authority/policy tuples for one subject unless a
future versioned authority policy explicitly defines cross-producer resolution.
...
The same subject, assertion, semantic effective point, authority, producer, policy, parents,
successor, and impact scope produce the same `availabilityRecordHash`. A status, producer,
conflict-resolution, or correction change creates a new record; prior records and hashes remain
unchanged.
