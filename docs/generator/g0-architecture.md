...
Public inputs must be ordinary plain-data objects with prototype exactly `Object.prototype` or `null`; arrays must use `Array.prototype`. Node's non-observing Proxy detection rejects Proxies before reflective inspection. Descriptor-only traversal copies the complete request, configuration, and catalog into deeply frozen arrays and null-prototype records without invoking accessors, inherited fields, iterators, coercion hooks, or Proxy traps. Custom prototypes, array subclasses, symbols, sparse arrays, custom properties, functions, cycles, and Proxies fail closed. No later phase retains or reads a caller-owned reference.

Admission derives the effective stage count, snapshotted mechanic count, and snapshotted `maxWorkUnits` from that one immutable graph. The exact requirement is compared with the snapshot budget before any callback or semantic validation. Insufficient budget fails with `maximum-work-units`; sufficient budget reserves exactly the required units and exposes only a frozen required/admitted/available/unused accounting record. Callback exceptions become the sanitized `callback-failed` error. The enforced phase order is safe-shape check, snapshot completion, work admission, callbacks, semantic validation, normalization, and generation. Callback mutation of the original request, nested limits, mechanic array/records, difficulty, checkpoint frequency, asset policy, or seed cannot affect output or hashes.

`maxWorkUnits`, available units, and unused units are execution metadata: the configuration hash projection excludes `limits.maxWorkUnits`, and no accounting field is written to the content-addressed GenerationBundle. N, N+1, and larger sufficient budgets therefore produce identical normalized requests, ObbySpecs, bundles, hashes, canonical bytes, and output names.
...
After locking, the CLI revalidates the parent ancestry, private-lock device/file identity, and final-path absence immediately before one atomic no-replace commit. Linux invokes `renameat2(RENAME_NOREPLACE)` through the committed, silent Python/ctypes bridge with fixed arguments and no shell. Windows invokes `MoveFileW` through the committed, silent PowerShell/P/Invoke bridge with fixed arguments and no shell. Both primitives fail when the destination exists; Linux, Windows, Python 3 on Linux, and Windows PowerShell are the supported runtime set. A missing helper/runtime, unsupported kernel/filesystem primitive, or other platform fails closed as `output-publication`; there is no ordinary-rename fallback.

Thus a public observer sees either no destination or the complete `generation-bundle.json`; there is no empty public claim and readers require no incomplete-state convention. The committed directory identity is checked against the former staging identity, the final and parent directories are synced, and the owned private lock is removed only after another identity check.

The private lock serializes conforming repository publishers, while the kernel no-replace primitive also protects against a non-conforming process racing the final syscall. A legitimate file or directory conflict returns `output-conflict`; a symlink, junction, or reparse conflict returns `path-safety`. Exact syscall-window tests preserve foreign identity and contents for empty/non-empty directories, files, and links, and prove no generated child is written through them. Replacing or renaming the private lock is detected before commit and cannot redirect a payload write. Cleanup removes staging and the lock only while their captured identities remain owned; foreign replacements are preserved. A crash may leave a private lock or staging directory, which fails closed without creating a public partial output.
